...
class StressTest:
    ...
    def generate_file(self, size_mb):
    ...
    def run_stress_test(self, operation, file_size_mb, client_pool_size, executor_mode='thread'):
        self.reset_counters()
        ...
        logging.info(f"Starting {operation} stress test with {file_size_mb}MB files, {client_pool_size} {executor_mode} workers")
        
        test_file = None
        if operation == 'upload' or operation == 'download':
            test_file = self.generate_file(file_size_mb)
        ...
        executor_class = ThreadPoolExecutor if executor_mode == 'thread' else ProcessPoolExecutor
        ...
        if not durations:
            logging.warning("Operations failed, no calculated statistics")
            return {
                'operation': operation,
    ...
    def run_all_tests(self, file_sizes, client_pool_sizes, server_pool_sizes, executor_modes, operations):
        all_stats = []
        
        #------ Waiting for the appropriate server ------#
                ...
                for operation in operations:
                    for file_size in file_sizes:
                        for client_pool_size in client_pool_sizes:
                            stats = self.run_stress_test(operation, file_size, client_pool_size, executor_mode)
                            if stats:
                                stats['server_pool_size'] = server_pool_size
                                all_stats.append(stats)
        
        self.save_to_csv(all_stats)
        
    def save_to_csv(self, all_stats):
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        csv_filename = f"stress_test_results_{timestamp}.csv"
        
        with open(csv_filename, 'w', newline='') as csvfile:
            fieldnames = [
                'operation', 'file_size_mb', 'client_pool_size', 'server_pool_size', 'executor_mode', 'avg_duration', 'avg_throughput', 'success_count', 'fail_count'
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--executor', choices=['thread', 'process', 'both'], default='thread', 
                        help='Executor mode (default: thread)')
    ...
    if len(operations) == 1 and len(file_sizes) == 1 and len(client_pool_sizes) == 1 and len(server_pool_sizes) == 1:
        ...
        stats = client.run_stress_test(operations[0], file_sizes[0], client_pool_sizes[0], executor_modes[0])
        if stats:
            stats['server_pool_size'] = server_pool_sizes[0]
            client.save_to_csv([stats])
    else:
        #------ Automate all test combinations ------#
        client.run_all_tests(file_sizes, client_pool_sizes, server_pool_sizes, executor_modes, operations)
